...
class SeoMozService
{
    ...
    public function urlMetrics($targets,$daily_history_deltas = [],$daily_history_values = [],$monthly_history_deltas = [],$monthly_history_values = [],$distributions = false) {
        ...
        $result =  json_decode($this->file_post_contents($requestUrl,json_encode($data)));
        return [
            'pageAuthority' => $result->results[0]->page_authority,
            'domainAuthority' => $result->results[0]->domain_authority,
            'linkingDomain' => $result->results[0]->root_domains_to_root_domain,
            'spamScore' => $result->results[0]->spam_score
        ];
    } 
